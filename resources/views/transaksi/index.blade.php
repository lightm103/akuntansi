        ...
        <div class="d-flex justify-content-between mb-4">
            <div>
                <button type="button" class="btn btn-primary " data-toggle="modal"
                    data-target="#pengeluaranModal">Tambah
                    Pengeluaran
                </button>
                <button type="button" class="btn btn-success ml-3" data-toggle="modal"
                    data-target="#pemasukanModal">Tambah
                    Pemasukan
                </button>
            </div>
            ...
            <div>
                <button type="button" class="btn btn-success ml-3" data-toggle="modal"
                    data-target="#eksportModal">Eksport Data
                </button>
                <a name="lihat-kas" id="lihat-kas" class="btn btn-primary ml-3" href="{{ route('kas.index') }}"
                    role="button">Lihat Kas</a>
            </div>
        </div>
            ...
            <div>
                <table>
                    <tr>
                        ...
                        <td>
                            <h5>{{ format_rupiah($totalTransactions['pemasukan']) }}</h5>
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 200px;">
                        ...
                        <td>
                            <h5> {{ format_rupiah($totalTransactions['pengeluaran']) }} </h5>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <tbody>
                @foreach ($transactions as $transaction)
                    <tr>
                        ...
                        <td>Rp{{ number_format($transaction->jenisTransaksi->kode_jenis_transaksi == 'debit' ? $transaction->jumlah : 0, 2, ',', '.') }}
                        </td>
                        <td>Rp{{ number_format($transaction->jenisTransaksi->kode_jenis_transaksi == 'kredit' ? $transaction->jumlah : 0, 2, ',', '.') }}
                        </td>
                        <td>
                            ...
                            <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"
                                data-target="#modalDelete_{{ $transaction->id }}">
                                Delete
                            </button>

                            {{-- Modal Delete --}}
                            <div class="modal fade" id="modalDelete_{{ $transaction->id }}" tabindex="-1"
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" name="tanggal_transaksi"
                                id="tanggal_transaksi" aria-describedby="helpId" placeholder="" required>
                        </div>
                        <div class="form-group">
                ...
                <form action="{{ route('transaksi.eksport') }}" method="POST" id="pengeluaranForm">
                    ...
                    <div class="modal-body">
                            ...
                            <select class="form-control" name="month" id="month">
                                ...
                                @foreach ($month as $date => $items)
                                    @foreach ($items as $item)
                                        @php
                                            $monthNumber = \Illuminate\Support\Carbon::parse($item->tanggal_transaksi)->format('m');
                                            $monthName = \Illuminate\Support\Carbon::parse($item->tanggal_transaksi)->format('M');
                                        @endphp

                                        @if (!in_array($monthNumber, $addedMonths))
                                            <option value="{{ $monthNumber }}">{{ $monthName }}</option>
                                            @php
                                                $addedMonths[] = $monthNumber;
                                            @endphp
                                        @endif
                                    @endforeach
                                @endforeach
                        ...
                        <div class="form-group">
                            ...
                            @foreach ($years as $date => $items)
                                @foreach ($items as $item)
                                    @php
                                        $yearsNumber = \Illuminate\Support\Carbon::parse($item->tanggal_transaksi)->format('y');
                                        $yearsName = \Illuminate\Support\Carbon::parse($item->tanggal_transaksi)->format('Y');
                                    @endphp

                                    @if (!in_array($yearsNumber, $addedYears))
                                        <option value="20{{ $yearsNumber }}">{{ $yearsName }}</option>
                                        @php
                                            $addedYears[] = $yearsNumber;
                                        @endphp
                                    @endif
                                @endforeach
                            @endforeach
        ...
        $(document).ready(function() {
            ...
            new DataTable('#table', {
                footerCallback: function(row, data, start, end, display) {
                    ...
                    var column5 = api.column(4, {
                        page: 'current'
                    }).data().reduce(function(a, b) {
                        return intVal(a) + intVal(b);
                    }, 0);
                    var column6 = api.column(5, {
                        page: 'current'
                    }).data().reduce(function(a, b) {
                        return intVal(a) + intVal(b);
                    }, 0);

                    $(api.column(4).footer()).html(column5.toLocaleString('id-ID', {
                        style: 'currency',
                        currency: 'IDR'
                    }));
                    $(api.column(5).footer()).html(column6.toLocaleString('id-ID', {
                        style: 'currency',
                        currency: 'IDR'
                    }));
                }
